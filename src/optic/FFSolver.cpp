...
namespace Planner
{
    ...
    //========================= Début Ajout ======================================
    // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    // [STN EXPORT] Exporte le plan final sous forme de STN au format JSON.
    // Appelée une fois par plan solution trouvé, depuis carryOnSearching(),
    // juste après FFEvent::printPlan(plan).
    //
    // Chaque nœud = une étape du plan avec ses bornes temporelles :
    //   earliestStart : borne inf (earliest time)
    //   latestStart : borne sup (latest time), null si non bornée
    //   scheduledTime    : timestamp retenu par le solveur
    //
    // Les arcs (contraintes de précédence) ne sont pas accessibles ici car
    // ils appartiennent au ChildData de LPScheduler qui n'est plus en vie.
    // Pour les arcs, voir l'export complet dans lpscheduler.cpp.
    // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    static void exportFinalPlanAsSTN(const list<FFEvent> &plan)
    {
        std::ofstream out("stn_final_plan.json");
        if (!out.is_open())
        {
            std::cerr << "[STN EXPORT] ERREUR : impossible de créer stn_final_plan.json\n";
            return;
        }
        ...
        bool first = true;
        int step = 0;
        for (list<FFEvent>::const_iterator it = plan.begin(); it != plan.end(); ++it, ++step)
        {
            // Ignorer les dummy steps internes
            if (it->isDummyStep()) { continue; }
            ...
            if (it->time_spec == Planner::E_AT)
        {
            out << "      \"action\": \"TIL_" << it->divisionID << "\",\n";
            out << "      \"time_spec\": \"E_AT\",\n";
            out << "      \"agents\": [],\n";
        }
        else
        {
            ...
            out << "      \"time_spec\": \""
                << (it->time_spec == Planner::E_AT_START ? "E_AT_START" : "E_AT_END")
                << "\",\n";

            // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            // [STN EXPORT] Extraction des agents par parsing du nom d'action.
            // On parse la string "(action-name param1 param2 ...)" produite
            // par operator<< de instantiatedOp, puis on croise chaque token
            // avec la liste des objets de type agent du problème PDDL.
            out << "      \"agents\": [";
            bool firstAgent = true;

            if (it->action)
            {
                // Construire la liste des noms d'objets de type "agent"
                // depuis la table des objets VAL
                set<string> agentNames;
                const VAL::const_symbol_list* objects =
                    VAL::current_analysis->the_problem->objects;
                if (objects)
                {
                    for (VAL::const_symbol_list::const_iterator oi = objects->begin();
                        oi != objects->end(); ++oi)
                    {
                        // Remonter la hiérarchie de types pour détecter "agent"
                        const VAL::pddl_type* t = (*oi)->type;
                        while (t)
                        {
                            if (t->getName() == "agent")
                            {
                                agentNames.insert((*oi)->getName());
                                break;
                            }
                            t = t->type;  // parent dans VAL s'appelle "type" pas "parent"
                        }
                    }
                }

                // Parser "(action-name token1 token2 ...)" et garder les agents
                std::ostringstream actionStream;
                actionStream << *(it->action);
                string actionStr = actionStream.str();

                // Supprimer les parenthèses
                size_t start = actionStr.find('(');
                size_t end   = actionStr.find(')');
                if (start != string::npos && end != string::npos)
                {
                    string inside = actionStr.substr(start + 1, end - start - 1);
                    std::istringstream iss(inside);
                    string token;
                    bool firstToken = true;  // ignorer le nom de l'action
                    while (iss >> token)
                    {
                        if (firstToken) { firstToken = false; continue; }
                        if (agentNames.count(token))
                        {
                            if (!firstAgent) out << ", ";
                            out << "\"" << token << "\"";
                            firstAgent = false;
            ...
            if (it->lpMinTimestamp == DBL_MAX)
                out << "      \"earliestStart\": null,\n";
            else
                out << "      \"earliestStart\": " << it->lpMinTimestamp << ",\n";

            if (it->lpMaxTimestamp == DBL_MAX)
                out << "      \"latestStart\": null,\n";
            else
                out << "      \"latestStart\": " << it->lpMaxTimestamp << ",\n";

            // out << "      \"scheduledTime\": " << it->lpTimestamp << "\n";
            // out << "    }";
        }
        ...
        std::cout << "[STN EXPORT] Plan final exporté -> stn_final_plan.json\n";
    }
    // ===================================== Fin de l'ajout =============================================
                                          ...
                                          bool &wasNewBestSolution)
    {
        ...
        if (metric > Globals::bestSolutionQuality + 0.01)
        {
            ...
            // ====================== Ajout CLAUX Jérémy ================
            // Export du plan solution complet en JSON STN.
            exportFinalPlanAsSTN(plan);
            // ========================== Fin Ajout =====================
