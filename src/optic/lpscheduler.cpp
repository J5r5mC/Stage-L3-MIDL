...
//===================== Ajout de CLAUX Jérémy =============================
// ============================================================
// [STN EXPORT] Fonction d'export du STN au format JSON
// Appelée depuis sortOutTheSolution() avec le vecteur planAsAVector
// et depuis ~ChildData() avec temporaryEdges + eventsWithFakes.
// ============================================================
static void exportSTNNodes(std::ostream &out,
                           const vector<FFEvent *> &plan,
                           bool &firstNode)
{
    for (int i = 0; i < (int)plan.size(); ++i) {
        const FFEvent *ev = plan[i];
        if (!ev) continue;

        // Ignorer les dummy steps internes (préférences, triggers...)
        if (ev->isDummyStep()) continue;
        ...
        // Nom de l'action ou TIL
        if (ev->time_spec == Planner::E_AT) {
            out << "      \"action\": \"TIL_" << ev->divisionID << "\",\n";
            out << "      \"time_spec\": \"E_AT\",\n";
        } else {
            if (ev->action) {
            ...
            out << "      \"time_spec\": \""
                << (ev->time_spec == Planner::E_AT_START ? "E_AT_START" : "E_AT_END")
                << "\",\n";
        }

        // Bornes temporelles issues du BF/LP
        if (ev->lpMinTimestamp == DBL_MAX)
            out << "      \"lpMinTimestamp\": null,\n";
        else
            out << "      \"lpMinTimestamp\": " << ev->lpMinTimestamp << ",\n";

        if (ev->lpMaxTimestamp == DBL_MAX)
            out << "      \"lpMaxTimestamp\": null,\n";
        else
            out << "      \"lpMaxTimestamp\": " << ev->lpMaxTimestamp << ",\n";

        out << "      \"lpTimestamp\": " << ev->lpTimestamp << "\n";
        out << "    }";
    }
}

static void exportSTNEdges(std::ostream &out,
                           const map<int, IncomingAndOutgoing> &edges,
                           bool &firstEdge)
{
    // Chaque entrée : edges[from].mustFollowThis() => {to -> (isEpsilon, distance)}
    // Signification : t_to >= t_from + distance  (ou + epsilon si isEpsilon)
    for (auto eItr = edges.begin(); eItr != edges.end(); ++eItr) {
        const int from = eItr->first;
        const map<int, pair<bool,double>> &followers = eItr->second.mustFollowThis();
        for (auto fItr = followers.begin(); fItr != followers.end(); ++fItr) {
            const int to        = fItr->first;
            const bool isEps    = fItr->second.first;
            const double dist   = fItr->second.second;
            ...
            out << "      \"to\": "   << to   << ",\n";
            out << "      \"minGap\": " << dist << ",\n";
            out << "      \"isEpsilonStrict\": " << (isEps ? "true" : "false") << "\n";
            out << "    }";
        }
    }
}

static int stnExportCounter = 0;  // pour nommer les fichiers par appel

static void exportSTN(const vector<FFEvent *> &plan,
                      const map<int, IncomingAndOutgoing> &edges)
{
    std::ostringstream filename;
    filename << "pddl/stn_export_" << stnExportCounter++ << ".json";

    std::ofstream out(filename.str().c_str());
    if (!out.is_open()) {
        std::cerr << "[STN EXPORT] Impossible d'ouvrir " << filename.str() << "\n";
        return;
    }
    ...
    out << "  \"nodes\": [\n";
    bool firstNode = true;
    exportSTNNodes(out, plan, firstNode);
    out << "\n  ],\n";

    // -- Arcs --
    out << "  \"edges\": [\n";
    bool firstEdge = true;
    exportSTNEdges(out, edges, firstEdge);
    out << "\n  ]\n";

    out << "}\n";
    out.close();

    std::cout << "[STN EXPORT] STN exporté dans " << filename.str() << "\n";
}
// =============================== Fin de l'ajout ===================================
                                         ...
                                         const map<int, double> &rpLowerBounds)
    {
        ...
        static const bool optimised = true;
        {
            ...
            // ================= Ajout de CLAUX Jérémy =======================
            // [STN EXPORT] Export des bornes temporelles (nœuds du STN)
            // Les timestamps lpMin/lpMax/lpTimestamp sont à jour à ce point.
            // Les arcs viennent du ChildData courant (cd) s'il existe.
            
            //static bool stnAlreadyExported = false;
            //if (!stnAlreadyExported) {
            //    static map<int, IncomingAndOutgoing> emptyEdges;
            //    const map<int, IncomingAndOutgoing> &stnEdges =
            //        (cd ? cd->getTemporaryEdges() : emptyEdges);
            //    exportSTN(planAsAVector, stnEdges);
            //    stnAlreadyExported = true;
            //}
            // ====================== Fin de l'ajout =========================
        }
